...
export default function FeedScreen() {
  ...
  const goNext = useCallback(() => {
    Animated.timing(translateY, { toValue: -SCREEN_H, duration: 250, useNativeDriver: true })
      .start(() => { setIndex(i => i + 1); translateY.setValue(0); });
  }, []);
  ...
  // PanResponder only handles the background layer — never sees the action buttons
  ...
  const handleLike = async () => {
    if (!current) return;
    if (!current.is_liked) { setLikeFlash(false); setTimeout(() => setLikeFlash(true), 10); }
    try {
      if (current.is_liked) {
  ...
  const handleDiscover = async (skipName = false) => {
    ...
    try {
      ...
      setStack(prev => prev.map((c, i) => i === index ? {
        ...c, custom_name: result.color.custom_name,
        discovered_by: result.color.discovered_by, discovered_at: result.color.discovered_at,
      } : c));
      setDiscoverModalVisible(false);
      setCustomName('');
      setPendingDiscover(false);
      if (result.is_first_discoverer) {
        Alert.alert('🎉 First Discovery!',
          `You discovered #${current.hex_code}${result.color.custom_name ? `\n"${result.color.custom_name}"` : ''}`,
          [{ text: 'Amazing!' }]);
      }
    } catch (e) { setDiscoverModalVisible(false); setPendingDiscover(false); }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── Layer 1: animated background + swipe gesture ── */}
      <Animated.View
      ...
      {/* ── Layer 2: UI overlay — completely separate from PanResponder ── */}
      <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
        ...
        {current.discovered_by && (
          <View style={[styles.discovererBadge, { top: insets.top + 50 }]} pointerEvents="none">
            <Text style={[styles.discovererText, { color: fg }]}>
              @{current.discovered_by.username}
            </Text>
            {current.custom_name && (
              <Text style={[styles.customNameText, { color: fg }]}>
                "{current.custom_name}"
              </Text>
            )}
          </View>
        ...
        <View style={[styles.actions, { bottom: insets.bottom + 100 }]}>
          <TouchableOpacity style={styles.actionBtn} onPress={handleLike} hitSlop={{ top: 10, bottom: 10, left: 16, right: 16 }}>
          ...
          <TouchableOpacity style={styles.actionBtn} onPress={() => setCommentsOpen(true)} hitSlop={{ top: 10, bottom: 10, left: 16, right: 16 }}>
            <Text style={styles.actionIcon}>💬</Text>
            <Text style={[styles.actionLabel, { color: fg }]}>{current.comments_count}</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.actionBtn} onPress={() => setDetailOpen(true)} hitSlop={{ top: 10, bottom: 10, left: 16, right: 16 }}>
        ...
        {index === 0 && stack.length <= 3 && (
          <View style={styles.swipeHint} pointerEvents="none">
            <Text style={[styles.swipeHintText, { color: fg }]}>
              swipe to explore ↑  ·  double-tap to like
            </Text>
          </View>
        )}
      </View>
      ...
      <CommentSheet
        ...
        onCountChange={(delta) =>
          setStack(prev => prev.map((c, i) =>
            i === index ? { ...c, comments_count: c.comments_count + delta } : c))
        }
      />
...
const styles = StyleSheet.create({
  ...
  customNameText:  { fontSize: 15, fontWeight: '700', marginTop: 2 },
  actions:    { position: 'absolute', right: 16, zIndex: 20, alignItems: 'center', gap: 24 },
